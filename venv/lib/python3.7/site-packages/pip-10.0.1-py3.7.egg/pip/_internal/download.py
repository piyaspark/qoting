...
class MultiDomainBasicAuth(AuthBase):
    ...
    def handle_401(self, resp, **kwargs):
        ...
        # Prompt the user for a Pipfile username and password
        username = six.moves.input("User for %s: " % parsed.netloc)
        password = getpass.getpass("Password: ")

        # Store the Pipfile username and password to use for future requests
        if username or password:
            self.passwords[parsed.netloc] = (username, password)

        # Consume content and release the original connection to allow our Pipfile
        #   request to reuse the same one.
        resp.content
        resp.raw.release_conn()

        # Add our Pipfile username and password to the request
        req = HTTPBasicAuth(username or "", password or "")(resp.request)

        # Send our Pipfile request
